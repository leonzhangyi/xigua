...
public class NumberUtils {
     ...
     * @return
     */
    public static BigDecimal add(int scale,String a,String... b) {
        BigDecimal res = new BigDecimal(a);
        for (String t : b) {
            if (null != res) {
                res = add(res, new BigDecimal(t), scale);
            }
        }
        return res;
    }
     ...
     * @return
     */
    public static boolean bigNumber(String big, String min) {
        if (!isNumeric(big) || !isNumeric(min)) {
            return false;
        }
        return Integer.parseInt(big) > Integer.parseInt(min);
    }
...
//        BigDecimal c=new BigDecimal("1.3");
        ...
        String b = "你好棒啊，小王";
        double days = Math.ceil(1.8);
        try {
            System.out.println(a.getBytes("UTF-8").length + "============" + b.getBytes("UTF-8").length);
